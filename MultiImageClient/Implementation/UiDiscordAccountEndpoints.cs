...
namespace MultiImageClient
{
    public static class UiDiscordAccountEndpoints
    {
        public static bool IsPublicRequest(string path, string method) =>
            method == "GET" && path is "/public/signup" or "/public/signup/claim"
            ...
            UiAccountActivity? activity = null, UiDiscordAccountRequests? requests = null)
        {
            ...
            app.MapPost("/public/signup/request", async (HttpContext ctx) =>
            {
                ...
                if (ctx.Request.ContentLength is null or > 2048 || !ctx.Request.HasFormContentType) return Results.BadRequest();
                try
                {
                    ...
                    return Results.Content(Page("Check Discord", "<p role=\"status\">" + E(result.Message)
                        + "</p><p><a href=\"" + E(SignupUrl()) + "\">Back to account request</a></p>"),
                        "text/html; charset=utf-8", statusCode: result.State == "failed" ? 502 : 200);
                }
                catch (Exception ex) when (ex is not OutOfMemoryException)
                {
                    var error = ex is InvalidOperationException ? ex.Message : "The account request failed. Try again later.";
                    return Results.Content(RequestPage(SignupUrl(), "", error), "text/html; charset=utf-8", statusCode: 400);
                }
            });
            app.MapGet("/public/signup/claim", (HttpContext ctx) =>
            {
                Headers(ctx, script: true);
        ...
        private static void Headers(HttpContext ctx, bool script = false)
        {
            ...
            ctx.Response.Headers.ContentSecurityPolicy = "default-src 'none'; style-src 'unsafe-inline'; form-action 'self'; frame-ancestors 'none'; base-uri 'none'"
                + (script ? "; connect-src 'self'; script-src 'sha256-" + Convert.ToBase64String(SHA256.HashData(Encoding.UTF8.GetBytes(ClaimScript))) + "'" : "");
        }
        ...
        public static string RequestPage(string signupUrl, string shareToken, string? error = null) => Page("Request an account",
            "<p>Enter your Discord username. The bot will send a link to that account if it can access #vibecoders.</p>"
            + (error == null ? "" : "<p role=\"alert\">" + E(error) + "</p>")
            + "<form method=\"post\" action=\"" + E(signupUrl + "/request") + "\"><label for=\"username\">Discord username</label>"
            ...
            + "<input type=\"hidden\" name=\"share\" value=\"" + E(shareToken) + "\"><button>Send me an account link</button></form>");
