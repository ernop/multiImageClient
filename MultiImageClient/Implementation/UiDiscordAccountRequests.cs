...
namespace MultiImageClient
{
    ...
    public sealed class UiDiscordAccountRequests
    {
        ...
        public async Task<Delivery> RequestAsync(string username, string ip, string shareToken, CancellationToken ct)
        {
            ...
            try
            {
                ...
                if (previous.Any(ticket => ticket.ExpiresAt > now && ticket.State is "pending" or "sent" or "activating" or "granting"))
                    return new("pending", "An account link is already pending. Check your Discord DMs. It expires after 30 minutes.");
                ...
                var ticket = new Ticket { Id = Guid.NewGuid().ToString("N"), TokenHash = Hash(secret),
                    UserId = member.UserId, Username = member.Username, GuildId = member.GuildId, ChannelId = member.ChannelId,
                    CreatedAt = now, ExpiresAt = now + (long)Lifetime.TotalMilliseconds, ShareToken = shareToken };
                doc.Tickets.Add(ticket);
                Write(doc); // Persist the recipient and pending send before either Discord POST.
                try
                {
                    await client.SendLinkAsync(member.UserId, PublicUrl(_settings, _auth, _environments) + "/claim#" + ticket.Id + "." + secret, ct);
                    ticket.State = "sent";
                    Write(doc);
                    return new("sent", "Check your Discord DMs for the account link. It expires in 30 minutes.");
                }
                ...
                {
                    return new("pending", "Discord delivery is unconfirmed. Check your DMs before requesting another link.");
                }
            }
        ...
        private Ticket RequireTicket(Document doc, string token)
        {
            ...
            if (ticket == null || ticket.ExpiresAt <= _now().ToUnixTimeMilliseconds()
                || ticket.State is "used" or "failed"
        ...
        private Document Read()
        {
            ...
            if (doc.Version != 1 || doc.Tickets.Count > 2048 || doc.Attempts.Count > 60
                ...
                || doc.Tickets.Any(ticket => !Regex.IsMatch(ticket.Id, @"\A[a-f0-9]{32}\z")
                    ...
                    || ticket.ExpiresAt != ticket.CreatedAt + (long)Lifetime.TotalMilliseconds
                    || ticket.State is not ("pending" or "sent" or "failed" or "activating" or "granting" or "used")
