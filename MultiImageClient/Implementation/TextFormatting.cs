...
namespace MultiImageClient
{
    public static class TextFormatting
    {
        ...
        public static async Task SaveImageAndAnnotateText(byte[] imageBytes, IEnumerable<PromptHistoryStep> historySteps, Dictionary<string, string> imageInfo, string outputPath)
        {
            using var ms = new MemoryStream(imageBytes);
            using var originalImage = Image.FromStream(ms);

            int annotationWidth = 1000; // Standard width for the right side annotation section
            using var annotatedImage = new Bitmap(originalImage.Width + annotationWidth, originalImage.Height);
            using var graphics = Graphics.FromImage(annotatedImage);
            
            // Draw the original image
            graphics.DrawImage(originalImage, 0, 0);
            
            // Set up the annotation area
