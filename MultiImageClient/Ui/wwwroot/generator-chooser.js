...
function buildGenChip(g) {
  const label = createGeneratorToggle(g, {
    checked: generatorPreferences.defaultSelectedKeys.includes(g.key),
    onChange: () => updateGeneratorCount(),
  });
  label.title = generatorChooserTitle(g.key, label.title);
  return label;
}
